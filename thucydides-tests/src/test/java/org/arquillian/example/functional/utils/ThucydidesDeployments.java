...
public class ThucydidesDeployments {
    public static WebArchive create() {
        final MavenDependencyResolver resolver = DependencyResolvers.use(MavenDependencyResolver.class)
            .loadMetadataFromPom("pom.xml")
            .goOffline();

        final Archive<?> webArchive = ShrinkWrap.createFromZipFile(WebArchive.class,
            resolver.artifacts("org.arquillian.example:beer-advisor-web:war:1.0.0-SNAPSHOT")
                .resolveAsFiles()[0]);
